      ...
      e.preventDefault();
                try {
                  ...
                  if(object.split(' ').length>1){
                    return toast.error("Please provide only single object name to remove");}
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input 
          ...
          type="file" 
          accept='image/*'
          className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-900' 
          required />
          <p className='mt-6 text-sm font-medium'>Describe your object name to remove</p>
        <textarea 
          rows={4}
          onChange={(e)=>setObject(e.target.value)} value={object}
          type="text"
          className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' 
          placeholder='e.g., ,Only single object name' 
          required />
